...
const mapSeed = (value: number, [destination, source, length]: [number, number, number]) => {
...
const mapSeedMultipleMaps = (value: number, ranges: ArrayTuple) => {
  let result = value;
...
export const getSeedRanges = (seeds: number[]) => {
  let chunked: number[][] = [];
  for (let i = 0; i < seeds.length; i += 2) {
    chunked.push(seeds.slice(i, i + 2));
  }
...
export default function testing(input: string) {
  ...
  const seedPairs = getSeedRanges(seeds);
